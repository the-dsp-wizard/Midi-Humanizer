...
int main() {
    ...
    int err = gradient_descent(neuralnet_model, // model function
        ...
        N, // parameter amount
        10, // examples amount
        1000, // batch size (lower or equal to sum of examples size, if zero or invalid will use full batch)
    ...
    for (int output = 0; output < out_neurons; output++) {
        for (int param = 0; param < (input_memory + output_memory + 1); param++) {
            if (param < input_memory)
                printf("l%d_x[%d] = %f\n", output, param, parameters[(input_memory + output_memory + 1) * output + param]);
            else if (param < (input_memory + output_memory) )
                printf("l%d_y[%d] = %f\n", output, param - input_memory, parameters[(input_memory + output_memory + 1) * output + param]);
            else
                printf("l%d_b = %f\n", output, parameters[(input_memory + output_memory + 1) * output + param]);

        }
    }

    for (int output = 0; output < out_neurons; output++) {
        printf("l%d_oc = %f\n", output, parameters[output + out_neurons * (input_memory + output_memory + 1)]);
    }

    printf("bias = %f\n", parameters[out_neurons + out_neurons * (input_memory + output_memory + 1)]);
